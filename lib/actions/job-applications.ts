...
export const updateJobApplication = async (
  ...
  if (isMovingToDifferentColumn) {
    ...
    if (order !== undefined && order !== null) {
      ...
      const jobsNeedingToShift = jobsInTargetColumn.slice(order);

      for (const job of jobsNeedingToShift) {
    ...
    const currentPostionIndex = otherJobsInColumn.findIndex(
      (job) => job.order > currentJobOrder,
    );

    const oldPositionIndex =
      currentPostionIndex === -1
